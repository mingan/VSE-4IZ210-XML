...
<xsl:template match="SHOP">
    ...
    <head>       
        <title>Nabídka produktů našeho eshopu</title>        
        <link rel="stylesheet" type="text/css" href="zbozifeed_style.css"/>
        <meta content="text/html; charset=UTF-8" http-equiv="content-type"/>
    </head>
    <body>
        <h1>Nabídka fair-trade certifikovaných produktů</h1>        
    ...
    <xsl:template match="SHOPITEM">
        ...
        <table>
            <tr><td rowspan="8" ><img src="{IMGURL}" alt=""/></td><td border="1">Popis</td><td><xsl:value-of select="DESCRIPTION"/></td></tr>
            <!-- XML dokument musí obsahovat PRICE nebo PRICE_VAT a VAT-->
            <xsl:if test="PRICE"><tr><td class="nazevParametru" border="1">Cena v Kč bez DPH</td><td><xsl:value-of select="PRICE"/></td></tr></xsl:if>                                    
            <xsl:if test="PRICE_VAT"><tr><td border="1">Cena v Kč s DPH</td><td><xsl:value-of select="PRICE_VAT"/></td></tr></xsl:if>
            <xsl:if test="VAT"><tr><td border="1">Sazba DPH</td><td><xsl:value-of select="VAT"/></td></tr></xsl:if>
            <tr>
                <xsl:variable name="serviceLife" select="SERVICE_LIFE"/>
                <td>Životnost</td>
                <td><xsl:analyze-string select="$serviceLife" regex="(\+?\d+)((e\+?)(\d+))?">
                <xsl:matching-substring>
                    <xsl:value-of select="regex-group(1)"/>
                    
                    <xsl:if test="string-length(regex-group(2))">
                        × 10<sup><xsl:value-of select="regex-group(4)"/></sup>
                    </xsl:if>
                </xsl:matching-substring>
